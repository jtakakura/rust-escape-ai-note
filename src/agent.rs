...
use crate::{IS_PLAY_SLEEP_ANIMATION, LAYER_DOOR, LAYER_WALLS, RESOURCES, TEXTURES};
...
impl Agent {
    ...
    pub fn draw(&self, scale_factor: f32, offset_x: f32, offset_y: f32) {
        ...
        let mut texture = match self.birth_ts.elapsed().as_secs() {
            0 => textures.agent_sleep1_texture.to_owned(),
            1 => textures.agent_sleep2_texture.to_owned(),
            2 => textures.agent_sleep3_texture.to_owned(),
            _ => textures.agent_texture.to_owned(),
        };

        if !IS_PLAY_SLEEP_ANIMATION {
            texture = textures.agent_texture.to_owned();
        }
